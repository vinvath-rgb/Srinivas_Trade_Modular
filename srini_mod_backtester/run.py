...
def main():
    ...
    with st.sidebar:
        ...
        strat = st.selectbox("Strategy", ["SMA Crossover", "RSI Mean Reversion", "SMA+RSI (Composite)"])
        ...
        if strat == "SMA Crossover":
            ...
            params.update({"lookback": lookback, "buy_lt": buy_lt, "sell_gt": sell_gt})

        else:  # SMA+RSI (Composite)
            fast = st.number_input("Fast SMA", min_value=5, max_value=200, value=20, step=1)
            slow = st.number_input("Slow SMA", min_value=10, max_value=400, value=100, step=1)
            rsi_lookback = st.number_input("RSI lookback", min_value=5, max_value=60, value=14, step=1)
            buy_lt = st.number_input("RSI Buy <", min_value=5.0, max_value=50.0, value=30.0, step=1.0)
            sell_gt = st.number_input("RSI Sell >", min_value=50.0, max_value=95.0, value=70.0, step=1.0)
            params.update({
                "fast": fast, "slow": slow,
                "rsi_lookback": rsi_lookback,
                "buy_lt": buy_lt, "sell_gt": sell_gt
            })
